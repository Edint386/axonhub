...
test('quota popover retains quota progress renderers', () => {
  const quotaBadges = read('components/quota-badges.tsx');

  assert.match(quotaBadges, /\b(?:UsageTimeBar|ProgressBar)\b/, 'quota bars should use a shared progress renderer');
});
...
test('grouped representatives render the group mode, standalone channels their own', () => {
  ...
  assert.match(badgesSource, /(?:groupedChannels|sortedChannels)\.map\(\(channel: ProviderQuotaChannel\) => \(\n\s*<QuotaRow key=\{channel\.id\} channel=\{channel\} effectiveMode=\{effectiveModeFor\(channel\)\}/);
