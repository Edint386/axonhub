...
const PRESERVE_UNSUPPORTED_TOOLS_SETTINGS_QUERY = `
  query PreserveUnsupportedToolsSettings {
...
const UPDATE_PRESERVE_UNSUPPORTED_TOOLS_SETTINGS_MUTATION = `
  mutation UpdatePreserveUnsupportedToolsSettings($input: UpdatePreserveUnsupportedToolsSettingsInput!) {
    updatePreserveUnsupportedToolsSettings(input: $input)
  }
`;

const FORWARD_UNSUPPORTED_TOOLS_ACROSS_FORMATS_SETTINGS_QUERY = `
  query ForwardUnsupportedToolsAcrossFormatsSettings {
...
const UPDATE_FORWARD_UNSUPPORTED_TOOLS_ACROSS_FORMATS_SETTINGS_MUTATION = `
  mutation UpdateForwardUnsupportedToolsAcrossFormatsSettings($input: UpdateForwardUnsupportedToolsAcrossFormatsSettingsInput!) {
    updateForwardUnsupportedToolsAcrossFormatsSettings(input: $input)
  }
`;

export interface PreserveUnsupportedToolsSettings {
  enabled: boolean;
}

export interface UpdatePreserveUnsupportedToolsSettingsInput {
  enabled: boolean;
}

export interface ForwardUnsupportedToolsAcrossFormatsSettings {
  enabled: boolean;
}

export interface UpdateForwardUnsupportedToolsAcrossFormatsSettingsInput {
  enabled: boolean;
}

export function usePreserveUnsupportedToolsSettings() {
  const { handleError } = useErrorHandler();

  return useQuery({
    queryKey: ['preserveUnsupportedToolsSettings'],
    queryFn: async () => {
      try {
        const data = await graphqlRequest<{ preserveUnsupportedToolsSettings: PreserveUnsupportedToolsSettings }>(
          PRESERVE_UNSUPPORTED_TOOLS_SETTINGS_QUERY
        );
        return data.preserveUnsupportedToolsSettings;
      } catch (error) {
        handleError(error, i18n.t('common.errors.internalServerError'));
        throw error;
      }
...
export function useUpdatePreserveUnsupportedToolsSettings() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (input: UpdatePreserveUnsupportedToolsSettingsInput) => {
      const data = await graphqlRequest<{ updatePreserveUnsupportedToolsSettings: boolean }>(
        UPDATE_PRESERVE_UNSUPPORTED_TOOLS_SETTINGS_MUTATION,
        { input }
      );
      return data.updatePreserveUnsupportedToolsSettings;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['preserveUnsupportedToolsSettings'] });
      toast.success(i18n.t('common.success.systemUpdated'));
    },
    onError: () => {
      toast.error(i18n.t('common.errors.systemUpdateFailed'));
...
export function useForwardUnsupportedToolsAcrossFormatsSettings() {
  const { handleError } = useErrorHandler();

  return useQuery({
    queryKey: ['forwardUnsupportedToolsAcrossFormatsSettings'],
    queryFn: async () => {
      try {
        const data = await graphqlRequest<{
          forwardUnsupportedToolsAcrossFormatsSettings: ForwardUnsupportedToolsAcrossFormatsSettings;
        }>(FORWARD_UNSUPPORTED_TOOLS_ACROSS_FORMATS_SETTINGS_QUERY);
        return data.forwardUnsupportedToolsAcrossFormatsSettings;
      } catch (error) {
        handleError(error, i18n.t('common.errors.internalServerError'));
        throw error;
      }
...
export function useUpdateForwardUnsupportedToolsAcrossFormatsSettings() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (input: UpdateForwardUnsupportedToolsAcrossFormatsSettingsInput) => {
      const data = await graphqlRequest<{ updateForwardUnsupportedToolsAcrossFormatsSettings: boolean }>(
        UPDATE_FORWARD_UNSUPPORTED_TOOLS_ACROSS_FORMATS_SETTINGS_MUTATION,
        { input }
      );
      return data.updateForwardUnsupportedToolsAcrossFormatsSettings;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['forwardUnsupportedToolsAcrossFormatsSettings'] });
      toast.success(i18n.t('common.success.systemUpdated'));
    },
    onError: () => {
      toast.error(i18n.t('common.errors.systemUpdateFailed'));
