...
// Mutate calls f(ctx, m).
func (f ChannelHealthProbeRunFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ChannelHealthProbeRunMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ChannelHealthProbeRunMutation", m)
}
