...
type ChannelHealthProbeRunFunc func(context.Context, *ent.ChannelHealthProbeRunQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f ChannelHealthProbeRunFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.ChannelHealthProbeRunQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.ChannelHealthProbeRunQuery", q)
}

// The TraverseChannelHealthProbeRun type is an adapter to allow the use of ordinary function as Traverser.
type TraverseChannelHealthProbeRun func(context.Context, *ent.ChannelHealthProbeRunQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseChannelHealthProbeRun) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseChannelHealthProbeRun) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ChannelHealthProbeRunQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.ChannelHealthProbeRunQuery", q)
}
