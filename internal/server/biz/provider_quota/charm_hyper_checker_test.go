...
func TestCharmHyper_CheckQuota_NegativeBalanceIsExhausted(t *testing.T) {
	httpClient := httpclient.NewHttpClientWithClient(&http.Client{
		Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			return &http.Response{
				StatusCode: http.StatusOK,
				Header:     make(http.Header),
				Body:       io.NopCloser(strings.NewReader(`{"balance":-1.5}`)),
			}, nil
		}),
	})

	quota, err := NewCharmHyperQuotaChecker(httpClient).CheckQuota(context.Background(), &ent.Channel{
