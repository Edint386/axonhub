...
func TestQuotaService_PastDurationMinute_IncludesUsageAtWindowEnd(t *testing.T) {
	...
	count, err := svc.requestCount(ctx, svc.ent.UsageLog.Query().Where(usagelog.APIKeyIDEQ(apiKeyID)), window)
	require.NoError(t, err)
	require.Equal(t, int64(1), count)

	usage, err := svc.usageAgg(ctx, svc.ent.UsageLog.Query().Where(usagelog.APIKeyIDEQ(apiKeyID)), window, true, true)
	require.NoError(t, err)
