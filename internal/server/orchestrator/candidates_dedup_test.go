...
func TestDefaultSelector_Select_LegacyFallbackExpandsDuplicateAliasMappings(t *testing.T) {
	ctx, client := setupTest(t)

	ch, err := client.Channel.Create().
		SetType(channel.TypeOpenai).
		SetName("Legacy Alias Fanout Channel").
		SetBaseURL("https://api.openai.com/v1").
		SetCredentials(objects.ChannelCredentials{APIKey: "test-key"}).
		SetSupportedModels([]string{"gpt-4o-mini", "gpt-4.1-mini"}).
		SetDefaultTestModel("gpt-4o-mini").
	...
	selector := NewDefaultSelector(channelService, modelService, systemService)
	result, err := selector.Select(ctx, &llm.Request{Model: "fast"})
	require.NoError(t, err)
