...
func applyAnalyticsDailyStatsDateBounds(s *sql.Selector, filter *AnalyticsFilter, startDay, endDay time.Time) {
	if filter == nil || filter.StartTime == nil {
		s.Where(sql.GTE(s.C(usagelog.FieldCreatedAt), startDay.UTC()))
	}

	if filter == nil || filter.EndTime == nil {
		s.Where(sql.LT(s.C(usagelog.FieldCreatedAt), endDay.AddDate(0, 0, 1).UTC()))
	}
}
